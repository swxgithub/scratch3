...
const { STRING } = require('../../extension-support/argument-type');
...
class AdditionRNNDemo {
  ...
  async train(iterations, batchSize, numTestExamples) {
    ...
    for (let i = 0; i < iterations; ++i) {
      ...
      const lossChart = {
        name: 'lossChart', styles: { height: '400px' }
      };
      tfvis.render.linechart(
          ...
          const accuracyChart = {
            name: 'accuracyChart', styles: { height: '400px' }
          };
      tfvis.render.linechart(
          accuracyChart,
...
class Scratch3AdditionRnn {
    ...
    rnnLayerSize = Number.parseInt(args.rnnLayerSize);
    digits = Number.parseInt(args.digits);
    trainingSize = Number.parseInt(args.trainingSize);
    rnnLayers = Number.parseInt(args.rnnLayers);
    batchSize = Number.parseInt(args.batchSize);
    trainIterations = Number.parseInt(args.trainIterations);
    numExamples = Number.parseInt(args.numExamples);
    rnnType = args.rnnType.toString();

    console.log(args.rnnType);
    console.log(digits);
   

    
    runAdditionRNNDemo(digits,trainingSize,rnnType,rnnLayers,rnnLayerSize,batchSize,trainIterations,numExamples);
    
}

}

module.exports = Scratch3AdditionRnn;